...
import {
  User, Mail, Phone, Save, X, Shield, Key, Bell, Globe, Palette, Monitor, Moon, Sun, Camera
} from 'lucide-react';
import { getCurrentUser, setCurrentUser } from '../../utils/auth';
import { getRoleDisplayName } from '../../utils/auth';
import { getAuthToken } from '../../utils/auth'; // ✅ Make sure this is imported
...
const fetchProfile = async () => {
  const token = getAuthToken(); // ✅ This is enough
  const storedUser = JSON.parse(localStorage.getItem('currentUser') || '{}');

  if (!token || !storedUser?.role) {
    console.error('Missing auth token or user role');
    return;
  }

  try {
    const res = await fetch(`http://localhost:3000/api/${storedUser.role}/profile`, {
      headers: {
    ...
    if (!res.ok || !res.headers.get('content-type')?.includes('application/json')) {
      throw new Error(`Invalid profile response (status ${res.status})`);
    }
...
useEffect(() => {
  ...
  // Optionally, you can reset loadingProfile to true when modal closes
