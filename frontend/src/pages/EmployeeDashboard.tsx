...
import {
  Clock, Calendar, TrendingUp, CheckCircle, FileText,
  Send, Plus, Target, AlertCircle
} from 'lucide-react';
import { getCurrentUser } from '../utils/auth';
import {
  formatDate, getDaysUntilDeadline, isOverdue
  ...
  const fetchData = async () => {
    try {
      ...
      const [tasksRes, leaveRes] = await Promise.all([
        fetch(`http://localhost:3000/api/employee/${user.id}/tasks`, { headers }),
        fetch(`http://localhost:3000/api/employee/${user.id}/leaves`, { headers })
      ]);
