...
import { storeAuthToken } from '../utils/auth'; // or wherever it's defined
...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  try {
    ...
    // Merge token into user object for easier access later
    const userWithToken = { ...user, token };

    // Save to localStorage
    localStorage.setItem('currentUser', JSON.stringify(userWithToken));
    console.log('🔐 Auth Token:', token); // ✅ Print the token for debugging
    storeAuthToken(token); // OR setAuthToken(token)
    localStorage.setItem('userRole', user.role || '');

    console.log('✅ Login successful:', userWithToken);

    // Callback to parent (e.g., to re-render authenticated layout)
    onLogin();
