...
import {
  Users, ClipboardList, Calendar, TrendingUp, CheckCircle, Briefcase,
  BarChart3, Target, UserCheck, Plus, Clock, AlertTriangle, MessageSquare,
  Award, Star, Eye, Edit, Trash2, Send, Filter, Search, UserPlus
} from 'lucide-react';

import { getCurrentUser } from '../utils/auth';
import {
  formatDate, getDaysUntilDeadline, isOverdue, getRelativeDate, formatDateTime
...
const ManagerDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [membersRes, tasksRes, leaveRes, reportsRes, projectsRes] = await Promise.all([
          fetch(`http://localhost:3000/api/managers/${user.id}/team-members`),
          fetch(`http://localhost:3000/api/managers/${user.id}/tasks`),
          fetch(`http://localhost:3000/api/managers/${user.id}/leave-requests`),
          fetch(`http://localhost:3000/api/managers/${user.id}/progress-reports`),
          fetch(`http://localhost:3000/api/managers/${user.id}/projects`)
        ]);

        const [members, tasks, leaves, reports, projects] = await Promise.all([
          membersRes.json(),
          tasksRes.json(),
          leaveRes.json(),
          reportsRes.json(),
  ...
  const pendingReports = progressReports.filter(r => !r.approved).length;
  ...
  const handleLeaveApproval = async (requestId: string, approved: boolean) => {
    try {
      ...
      setLeaveRequests(prev => prev.map(lr =>
        lr.id === requestId ? { ...lr, status: approved ? 'approved' : 'rejected' } : lr
      ));
  ...
  const handleProgressApproval = async (reportId: string, approved: boolean, comments?: string) => {
    try {
      await fetch(`/api/progress-reports/${reportId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ approved, comments })
      });
      setProgressReports(prev => prev.map(r =>
        r.id === reportId ? { ...r, approved } : r
      ));
  ...
  const handleAddEmployee = (newEmployee: any) => {
    ...
    // TODO: POST to backend if required
  };
